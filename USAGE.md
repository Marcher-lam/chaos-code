...
*   **流式 Markdown 渲染**：AI 回复实时流式输出，支持代码块语法高亮（12+ 语言）、表格对齐、粗体/斜体/链接等完整 Markdown 渲染。
*   **Tab 补全**：三层智能补全：`/` 开头匹配斜杠命令、`/model ` 后匹配模型名、`./` `../` `~/` 匹配文件路径。
*   **多行输入**：输入 `”””` 或 ``` 进入多行模式，支持括号/引号未闭合自动检测，空行提交。
*   **动态 Prompt**：提示符显示当前 git 分支名（脏标记 `*`）和活跃模型名，如 `main*:gpt-4o > `。
*   **实时工具调用展示**：AI 调用工具时显示结构化盒子（工具名+参数摘要+计时），流式输出中检测到工具名即刻显示 spinner。
