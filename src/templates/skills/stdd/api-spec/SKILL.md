...
## Purpose
从 BDD 规格生成**语言无关的 OpenAPI 规范**以及**多语言的类型定义、Mock 代码和验证器**。这是 Chaos Code 的 Spec-First + TDD CLI skill，支持任何编程语言和框架。
...
```bash
chaos api-spec <change-id> --language ts --full
```
...
#### Python (--language py)
```bash
chaos api-spec <change-id> --language py --full
```
...
#### Java (--language java)
```bash
chaos api-spec <change-id> --language java --full
```
...
#### Go (--language go)
```bash
chaos api-spec <change-id> --language go --full
```
...
#### C# (--language csharp)
```bash
chaos api-spec <change-id> --language csharp --full
```
...
#### Rust (--language rust)
```bash
chaos api-spec <change-id> --language rust --full
```
...
#### PHP (--language php)
```bash
chaos api-spec <change-id> --language php --full
```
...
# 自动检测语言并生成完整产物
chaos api-spec <change-id> --full

# 指定目标语言
chaos api-spec <change-id> --language python
chaos api-spec <change-id> --lang go

# 生成特定类型的产物
chaos api-spec <change-id> --language rust --types-only
chaos api-spec <change-id> --language java --mocks-only

# 指定输出目录
chaos api-spec <change-id> --output-dir src/api

# 指定框架/库 (取自 Orval + OpenAPI Generator)
chaos api-spec <change-id> --language ts --framework react-query  # React Query hooks
chaos api-spec <change-id> --language ts --framework swr          # SWR hooks
chaos api-spec <change-id> --language ts --framework vue-query    # Vue Query hooks
chaos api-spec <change-id> --language ts --framework angular      # Angular HttpClient
chaos api-spec <change-id> --language py --framework fastapi     # FastAPI 路由
chaos api-spec <change-id> --language java --framework spring     # Spring Boot

# 自定义模板 (取自 OpenAPI Generator)
chaos api-spec <change-id> --template-dir ./templates

# Monorepo workspace
chaos api-spec <change-id> --workspace packages/api

# 验证现有 API spec 与 BDD 一致性
chaos api-spec <change-id> --validate-existing

# 输出格式
chaos api-spec <change-id> --format json

# 集成插件 (取自 OpenAPI Generator)
chaos api-spec <change-id> --plugin maven       # Maven 插件集成
chaos api-spec <change-id> --plugin gradle      # Gradle 插件集成
chaos api-spec <change-id> --plugin bazel       # Bazel 插件集成

# IoC 模式 (取自 OpenAPI Generator)
chaos api-spec <change-id> --ioc                # Inversion of Control，保留业务逻辑代码
```
