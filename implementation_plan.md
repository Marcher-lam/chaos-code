...
* **目标**：实现页面截图的比对与分析。
* **物理位置**：`src/utils/visual-regression.js`
* **关键能力**：
...
* **物理位置**：`src/cli/commands/browser.js`
* **变更点**：
...
* **物理位置**：`src/cli/commands/verify.js`
* **变更点**：
...
* **物理位置**：`src/cli/commands/user-test.js`
* **变更点**：
...
* **物理位置**：`src/utils/css-extractor.js`
* **职责**：
...
* **物理位置**：`src/cli/commands/design.js`
* **变更点**：
...
* **物理位置**：`src/utils/workflow-dsl-interpreter.js`
* **职责**：
...
* **物理位置**：`src/runtime/agent-simulator.js`
* **职责**：
...
* **物理位置**：`src/utils/command-runner.js`
* **职责**：
...
* 运行全量 Jest 校验，确保原有 197 个套件、4151+ 条测试用例全部通过，保持 97% 以上的整体覆盖率。
```bash
npm run lint
npm run test:all
```
