...
**测试基线**: `npm run premerge`，覆盖 `npm audit --audit-level=high`、ESLint、文档契约测试和 Jest 回归测试（199 套件）。
...
| **artifact-registry.js** | 产物注册表 | 变更产物文件注册、索引与查询 |
| **change-helpers.js** | 变更辅助函数 | 变更名称生成、安全文件名转换、workspace 上下文解析 |
| **css-extractor.js** | CSS 样式提取 | 扫描 CSS/SCSS/Tailwind，提取设计令牌（色彩、字体、间距等） |
| **evidence-ledger.js** | 证据账本 | 结构化证据记录追加、查询与汇总 |
| **profile-engine.js** | 规划配置引擎 | 自评问卷、分数计算、Profile 生命周期管理 |
| **visual-regression.js** | 视觉回归比对 | 页面截图对比、Baseline 管理、Diff Ratio 计算 |
| **workflow-dsl-interpreter.js** | 工作流 DSL 解析 | 读取 YAML 工作流定义，Kahn 拓扑排序编译 DAG |
| **workspace-cache.js** | 工作区缓存 | Workspace 检测结果缓存、失效刷新 |
