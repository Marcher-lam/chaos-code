...
program.command('agent [goal...]')
  ...
  .option('--shell-run <command>', 'Run a bounded shell command through shell.run')
  .option('--fix-packet', 'Build an agent fix packet from current git/test context')
  .option('--write-prompt', 'Write fix packet JSON and Markdown prompt files')
  .option('--force', 'Overwrite files for initialization commands')
  ...
  .option('--repair', 'Run repair cycle with a fix patch and tests')
  .option('--llm-diff', 'Generate a candidate unified diff from a fix-packet prompt')
  .option('--llm-repair', 'Generate a diff from a prompt, preview it, then run repair cycle')
  .option('--write-report', 'Write agent run report JSON and Markdown files')
  .option('--history', 'List agent run reports')
  .option('--show-run <runId>', 'Show an agent run report')
  .option('--resume <runId>', 'Return resume context and suggested next command for a run')
  .option('--prompt <file>', 'Prompt file for --llm-diff')
  .option('--output <file>', 'Output diff path for --llm-diff', 'repair.diff')
  .option('--model <model>', 'Model override for --llm-diff')
  .option('--mock-response <text>', 'Mock LLM response for local/testing --llm-diff')
